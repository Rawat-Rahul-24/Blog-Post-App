...
export default class Search{
    constructor(){
       ...
       this.closeSearch = document.querySelector(".close-live-search")
    ...
    events(){
        ...
        this.closeSearch.addEventListener("click", () => this.closeOverlay())
    ...
    renderResultsHtml(posts) {
        if(posts.length){
            ...
            ${posts.map((post) => {
              ...
              <span class="text-muted small">${post.author.username} on ${postDate.getDate()}/${postDate.getMonth() + 1}/${postDate.getFullYear()}</span>
            </a>`
            }).join('')}
          </div>`)
        }else {
            this.resultArea.innerHTML = `<p class="alert alert-danger text-center shadow-sm">Sorry, we could not find any results</p>`
        }
        this.hideLoaderIcon()
